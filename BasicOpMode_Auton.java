...
@Autonomous(name = "AutonTest5.0.0")

//@Disabled
public class BasicOpMode_Auton extends OpMode{
    ...
    private double YWheelCircum = 5 * Math.PI;
    ...
    public int xrailYMovement(int distance) { //if positive, forwards. if negative, backwards
        double circumTraveled = distance / YWheelCircum;
        ...
        stringWheel.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER); //RunMode is an enum, a construct that defines
        //possible values, like STOP_AND_RESET_ENCODER
        stringWheel.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        stringWheel.setTargetPosition(position);
    ...
    public void init() {
        ...
        stringWheel = hardwareMap.dcMotor.get("string_wheel");
    ...
    public void loop() {
        ...
        switch(step) {
            ...
            case 2:
                ...
                break;
            case 3:
                flReached = Math.abs(stringWheel.getCurrentPosition()) >= Math.abs(targetPosition) - 20; //if it's negative it will never stop... but targetPosition would be positive
                if (flReached) {
                    step = nextStep;
                }
                break;
            case 4:
                targetPosition = xrailYMovement(10);
                nextStep = 5;
                step = 3;
                break;
        ...
        telemetry.addLine("XrailY: " + stringWheel.getCurrentPosition());
        telemetry.update();
